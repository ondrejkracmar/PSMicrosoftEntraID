...
title: Get-PSEntraIDContactDelta
---
...
```
Get-PSEntraIDContactDelta [[-DeltaSession] <hashtable>] [-MinimalDelta] [-EnableException]
```
...
Wraps Graph's contacts/delta endpoint.
...
Get-PSEntraIDContactDelta -DeltaSession $delta | Where-Object Removed

Subsequent call: only what changed, filtered down to what was removed.

### EXAMPLE 3

$delta = Import-PSEntraIDDeltaSession -Path .\contact.delta.json
PS C:\> Get-PSEntraIDContactDelta -DeltaSession $delta
...
### PSMicrosoftEntraID.Contacts.ContactDelta
