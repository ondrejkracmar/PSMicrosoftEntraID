...
Retrieves only the groups that changed since the previous call.
...
```
Get-PSEntraIDGroupDelta [[-DeltaSession] <hashtable>] [-MinimalDelta] [-EnableException]
```
...
Wraps Graph's groups/delta endpoint.
...
$delta = @{}
PS C:\> Get-PSEntraIDGroupDelta -DeltaSession $delta

First call: every group, and the token is stored in $delta.

### EXAMPLE 2

Get-PSEntraIDGroupDelta -DeltaSession $delta | Where-Object Removed

Subsequent call: only what changed, filtered down to what was removed.

### EXAMPLE 3

$delta = Import-PSEntraIDDeltaSession -Path .\group.delta.json
PS C:\> Get-PSEntraIDGroupDelta -DeltaSession $delta
PS C:\> Export-PSEntraIDDeltaSession -Path .\group.delta.json -DeltaSession $delta
...
### PSMicrosoftEntraID.Groups.GroupDelta
