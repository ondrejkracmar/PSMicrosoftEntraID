...
title: Remove-PSEntraIDMachineTag
---

# Remove-PSEntraIDMachineTag

## SYNOPSIS

Remove a tag from a Microsoft Defender for Endpoint machine.
...
```
Remove-PSEntraIDMachineTag -InputObject <Machine[]> -Tag <string[]> [-EnableException] [-Force]
...
```
Remove-PSEntraIDMachineTag -MachineId <string[]> -Tag <string[]> [-EnableException] [-Force]
 [-PassThru] [-WhatIf] [-Confirm]
```
...
Removes a machine tag via the MDE machines API (POST /machines/{id}/tags,
service family Endpoint).
Requires the WindowsDefenderATP permission
...
Get-PSEntraIDMachine -DeviceId $device.DeviceId | Remove-PSEntraIDMachineTag -Tag 'vip'

Removes the tag from every MDE machine record of the device.
...
```yaml
Type: PSMicrosoftEntraID.DefenderEndpoint.Machine[]
DefaultValue: ''
SupportsWildcards: false
Aliases: []
ParameterSets:
