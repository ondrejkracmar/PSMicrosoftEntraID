...
title: Get-PSEntraIDDeviceDelta
---
...
```
Get-PSEntraIDDeviceDelta [[-DeltaSession] <hashtable>] [-MinimalDelta] [-EnableException]
```
...
Wraps Graph's devices/delta endpoint.
...
Get-PSEntraIDDeviceDelta -DeltaSession $delta | Where-Object Removed

Subsequent call: only what changed, filtered down to what was removed.

### EXAMPLE 3

$delta = Import-PSEntraIDDeltaSession -Path .\device.delta.json
PS C:\> Get-PSEntraIDDeviceDelta -DeltaSession $delta
...
### PSMicrosoftEntraID.Devices.DeviceDelta
