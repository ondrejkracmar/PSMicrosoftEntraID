...
Retrieves only the users that changed since the previous call.
...
```
Get-PSEntraIDUserDelta [[-DeltaSession] <hashtable>] [-MinimalDelta] [-EnableException]
```
...
Wraps Graph's users/delta endpoint.
...
First call: every user, and the token is stored in $delta.

### EXAMPLE 2

Get-PSEntraIDUserDelta -DeltaSession $delta | Where-Object Removed

Subsequent call: only what changed, filtered down to what was removed.

### EXAMPLE 3

$delta = Import-PSEntraIDDeltaSession -Path .\user.delta.json
PS C:\> Get-PSEntraIDUserDelta -DeltaSession $delta
PS C:\> Export-PSEntraIDDeltaSession -Path .\user.delta.json -DeltaSession $delta
...
### PSMicrosoftEntraID.Users.UserDelta
