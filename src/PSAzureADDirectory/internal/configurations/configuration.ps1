...
$script:ModuleName = 'PSAzureADDirectory'
...
Set-PSFConfig -Module $script:ModuleName -Name 'Settings.GraphApiQuery.Select.UserLicense' -Value @('id','userPrincipalName','userType','accountEnabled','displayName','mail','companyName','assignedLicenses') -Initialize -Validation 'stringarray' -Description "Specifies filter select parameter to return a set of properties that are different than the default set for an individual resource or a collection of resources."
...
Set-PSFConfig -Module $script:ModuleName -Name 'Template.AzureADDirectory.UsageLocation' -Value $usageLocationTemplate -Initialize -Validation 'string' -Description "Location of json template of Azure Active Directory usage location."
