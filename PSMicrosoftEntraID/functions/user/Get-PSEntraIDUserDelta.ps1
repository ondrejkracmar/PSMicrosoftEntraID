function Get-PSEntraIDUserDelta {
    <#
    .SYNOPSIS
        Retrieves only the users that changed since the previous call.

    .DESCRIPTION
        Wraps Graph's users/delta endpoint. The first call with a given delta session
    ...
    .EXAMPLE
        PS C:\> Get-PSEntraIDUserDelta -DeltaSession $delta | Where-Object Removed

        Subsequent call: only what changed, filtered down to what was removed.

    .EXAMPLE
        PS C:\> $delta = Import-PSEntraIDDeltaSession -Path .\user.delta.json
        PS C:\> Get-PSEntraIDUserDelta -DeltaSession $delta
    ...
    #>
    [OutputType('PSMicrosoftEntraID.Users.UserDelta')]
    [CmdletBinding()]
    param (
        [Parameter()]
