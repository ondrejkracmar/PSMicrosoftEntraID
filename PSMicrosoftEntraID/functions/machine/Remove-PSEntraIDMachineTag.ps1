function Remove-PSEntraIDMachineTag {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        PS C:\> Get-PSEntraIDMachine -DeviceId $device.DeviceId | Remove-PSEntraIDMachineTag -Tag 'vip'
    ...
    process {
        ...
        foreach ($machineIdItem in $machineIdList) {
            foreach ($tagItem in $Tag) {
                [hashtable] $body = @{ Value = $tagItem; Action = 'Remove' }
                $response = Invoke-PSFProtectedCommand -ActionString 'MachineTag.Remove' -ActionStringValues $tagItem, $machineIdItem -Target (Get-PSFLocalizedString -Module $script:ModuleName -Name Identity.Platform) -ScriptBlock {
