function Get-PSEntraIDGroupDelta {
    <#
    .SYNOPSIS
        Retrieves only the groups that changed since the previous call.

    .DESCRIPTION
        Wraps Graph's groups/delta endpoint. The first call with a given delta session
    ...
    .EXAMPLE
        PS C:\> Get-PSEntraIDGroupDelta -DeltaSession $delta | Where-Object Removed

        Subsequent call: only what changed, filtered down to what was removed.

    .EXAMPLE
        PS C:\> $delta = Import-PSEntraIDDeltaSession -Path .\group.delta.json
        PS C:\> Get-PSEntraIDGroupDelta -DeltaSession $delta
    ...
    #>
    [OutputType('PSMicrosoftEntraID.Groups.GroupDelta')]
    [CmdletBinding()]
    param (
        [Parameter()]
